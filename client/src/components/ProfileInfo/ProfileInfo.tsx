...
export default function ProfileInfo({
  ...
  return (
    <>
        ...
        {!user.photo ? (
          ...
          >
            ...
            >
              ...
              {authUser?.id === user.id ? (
                <div>
                  Ваш профиль может не заинтересовать других пользователей, если
                  не загрузите фотографию
                </div>
              ) : (
          ...
          <img src={user.photo} alt="avatar" style={{ width: "45%" }} />
        )}
        {user.about ? (
          <Box
            sx={{
            ...
            {genres.length ? (
              genres.map((el) => (
                <Chip
                  color="secondary"
                  key={`g${el.id}`}
                  label={el.GenreTag?.genre}
                  sx={{ margin: "1%" }}
                />
              ))
            ) : (
              <Box>Ничего не выбрано</Box>
            )}
          </Box>
        </Box>
        <Box>
            ...
            {instruments.length ? (
              instruments.map((el) => (
                <Chip
                  color="secondary"
                  key={`i${el.id}`}
                  label={el.InstrumentTag?.instrument}
                  sx={{ margin: "1%" }}
                />
              ))
            ) : (
      ...
      <Box sx={{ width: "75%", marginBottom: "15%" }}>
        <h2>Песни</h2>
        {user.Songs?.length > 0 && (
          <>
            <div
        ...
        {authUser?.id !== user.id && user.Songs?.length === 0 && (
          <Box
            sx={{
              backgroundColor: "#282828",
            }}
